...
const sampleCategories = [
  { id: 1, name: "Plomberie", name_ar: "السباكة", name_fr: "Plomberie" },
  { id: 2, name: "Électricité", name_ar: "الكهرباء", name_fr: "Électricité" },
  { id: 3, name: "Peinture", name_ar: "الطلاء", name_fr: "Peinture" },
  { id: 4, name: "Quincaillerie", name_ar: "الأدوات المعدنية", name_fr: "Quincaillerie" },
  { id: 5, name: "Jardinage", name_ar: "البستنة", name_fr: "Jardinage" }
];
...
const sampleProducts = [
  {
    id: 1,
    name: "Marteau 500g",
    description: "Marteau de 500g avec manche en bois",
  ...
  },
  {
    id: 2,
    name: "Tuyau PVC 32mm",
    description: "Tuyau PVC diamètre 32mm longueur 3m",
    category: "Plomberie",
    category_id: 1,
    sku: "TUY001",
  ...
  },
  {
    ...
    description: "Câble électrique section 2.5mm² longueur 100m",
  ...
  {
    id: 4,
    name: "Peinture blanche mat 10L",
    description: "Peinture acrylique blanche mate 10 litres",
    category: "Peinture",
    category_id: 3,
    sku: "PEI001",
...
const initializeDatabase = async () => {
  try {
    // Create suppliers table (from sales system)
    await pool.execute(`
    ...
    // NOTE: Sales table is managed by jamalbrico system
    // ProductsManager only manages products, categories, and inventory
    // Sales are recorded by jamalbrico and inventory is updated via API calls
...
app.get("/api/stats", async (req, res) => {
  try {
    if (!useDatabase) {
      ...
      const totalStockValue = sampleProducts.reduce((sum, product) =>
        sum + (product.selling_price * product.remaining_stock), 0
      );
      const lowStockCount = sampleProducts.filter(product =>
        product.remaining_stock <= product.min_stock_level
      ).length;
    ...
    const [totalProductsResult] = await dbQuery.execute(
      "SELECT COUNT(*) as total FROM products"
    );

    // Get total stock value for all products
    const [totalValueResult] = await dbQuery.execute(
      "SELECT SUM(selling_price * remaining_stock) as total_value FROM products WHERE selling_price IS NOT NULL AND remaining_stock IS NOT NULL"
    );

    // Get low stock products count
    const [lowStockResult] = await dbQuery.execute(
      "SELECT COUNT(*) as low_stock_count FROM products WHERE remaining_stock <= 10"
    );
...
app.get("/api/integration/product/:identifier", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      product: salesCompatibleProduct
    });
  } catch (error) {
...
app.get("/api/integration/availability/:product_id", async (req, res) => {
  try {
    ...
    const rows = await dbQuery.execute(
      "SELECT id, name, remaining_stock, stock_quantity, min_stock_level, is_active FROM products WHERE id = ?",
      [product_id]
    );
...
app.get("/api/integration/availability/:productId", async (req, res) => {
  try {
    ...
    const availability = {
      ...
      is_active: product.is_active,
      stock_status: product.remaining_stock > product.min_stock_level ? 'good' :
                   product.remaining_stock > 0 ? 'low' : 'out_of_stock'
    };
...
app.post("/api/integration/sale", async (req, res) => {
  ...
  try {
    ...
    if (product.remaining_stock < quantity) {
      ...
      return res.status(400).json({
        success: false,
        error: `Insufficient stock. Available: ${product.remaining_stock}, Requested: ${quantity}`
      });
    }
    ...
    await connection.execute(`
      INSERT INTO sales (
        sale_number, date, customer_id, product_id, productName,
        price, quantity, category, totalPrice, discount, tax_amount,
        payment_method, notes
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      saleNumber, saleDate, customer_id, product_id, product.name,
      price, quantity, product.category_name || 'General', totalPrice,
      discount, tax_amount, payment_method, notes
    ]);
    ...
    res.json({
      ...
      inventory_update: {
        ...
        new_stock: updatedProduct.remaining_stock,
...
app.get("/api/integration/low-stock", async (req, res) => {
  try {
    ...
    const lowStockProducts = rows.map(product => ({
      ...
      selling_price: product.selling_price,
      unit: product.unit || 'piece',
      status: product.remaining_stock === 0 ? 'out_of_stock' :
              product.remaining_stock <= product.min_stock_level / 2 ? 'critical' : 'low',
      stock_deficit: Math.max(0, product.min_stock_level - product.remaining_stock)
    }));
    ...
    res.json({
      ...
      summary: {
        out_of_stock: lowStockProducts.filter(p => p.status === 'out_of_stock').length,
...
async function testConnection() {
  try {
    ...
    console.error('3. Create database: CREATE DATABASE jamalbrico;');
...
async function startServer() {
  try {
    ...
    app.get("/api/integration/products/search", async (req, res) => {
      try {
        ...
        const products = await dbQuery.execute(searchQuery, [
          searchTerm, searchTerm, searchTerm, searchTerm, searchTerm, searchTerm,
          searchTerm, exactQuery, exactQuery,
          limitNum
        ]);
    ...
    app.post("/api/integration/products/:id/update-stock", async (req, res) => {
      try {
        ...
        const currentProduct = await dbQuery.execute(
          "SELECT remaining_stock, name FROM products WHERE id = ? AND is_active = TRUE",
          [productId]
        );
        ...
        await dbQuery.execute(
          "UPDATE products SET remaining_stock = ?, updated_at = NOW() WHERE id = ?",
          [newStock, productId]
        );
        ...
        `, [
          ...
          `Stock ${operation === 'subtract' ? 'reduced' : 'increased'} via sales system integration`
        ]);
    ...
    app.listen(PORT, () => {
      ...
      console.log(`📊 MySQL Database: ${process.env.DB_NAME || "jamalbrico"}`);
